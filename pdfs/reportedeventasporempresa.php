...
while ($EMP = mysql_fetch_assoc($resultEMPRESAS)) {
    ...
    $sqlVentas="select * from ordendecompra where estatus=1 and conpago=1 and fechaderegistro>='".$_POST["id-date-picker-1"]."' and fechaderegistro<='".$_POST["id-date-picker-2"]."' and idempresa='".$EMP["idempresa"]."' order by fechaderegistro ";
    $resultVentas = mysql_query($sqlVentas, $con) or die(mysql_error());
    $numeroElementos=  mysql_num_rows($resultVentas);    
