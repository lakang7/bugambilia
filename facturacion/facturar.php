<?php
    ...
    $archivosPDF = array();
    $directorio = opendir("C:/xampp/htdocs/bugambilia/facturacion/salidapdf/".$configuracion["carpetabusqueda"]."/"); //ruta actual
    ...
    $archivosXML = array();    
    $directorio = opendir("C:/xampp/htdocs/bugambilia/facturacion/salidaxml/".$configuracion["carpetabusqueda"]."/"); //ruta actual
    while ($archivo = readdir($directorio)) //obtenemos un archivo y luego otro sucesivamente
    {   
        if($archivo!="." && $archivo!=".."){
    ...
    $RFCfacturacion=$configuracion["carpetabusqueda"];
    $RFCReceptor="";
    if($empresa["idpais"]==1){
    ...
    if($pais["idpais"]==1){
        if($orden["codigoexterno"]!=NULL){
            fwrite($file, "|COMPROBANTE|3.2|Sin Serie|".date("Y")."-".date("m")."-".date("d")." ".  date("H").":".date("i").":".date("s")."|Pago en Una Sola Exhibicion|".$orden["subtotal"]."|".$orden["total"]."|Transferencia Electrónica|Ingreso|USD|".$configuracion["cambio"]."||".$iva."||100|El cambio de Dolares Americanos a Pesos Mexicanos es ".$configuracion["cambio"].", Orden de Compra: ".$orden["codigoexterno"].", Orden de Producción: ".$orden["codigoop"].". ".$orden["comfacturacion"]."|FALSE|micorreo@pruebascorreo.com|||FACTURA|".$empresa["ultimos"]."|" . PHP_EOL);
        }else{
            fwrite($file, "|COMPROBANTE|3.2|Sin Serie|".date("Y")."-".date("m")."-".date("d")." ".  date("H").":".date("i").":".date("s")."|Pago en Una Sola Exhibicion|".$orden["subtotal"]."|".$orden["total"]."|Transferencia Electrónica|Ingreso|USD|".$configuracion["cambio"]."||".$iva."||100|El cambio de Dolares Americanos a Pesos Mexicanos es ".$configuracion["cambio"].", Orden de Producción: ".$orden["codigoop"].". ".$orden["comfacturacion"]."|FALSE|micorreo@pruebascorreo.com|||FACTURA|".$empresa["ultimos"]."|" . PHP_EOL);
        }
    } else {
        if($orden["codigoexterno"]!=NULL){
            fwrite($file, "|COMPROBANTE|3.2|Sin Serie|".date("Y")."-".date("m")."-".date("d")." ".  date("H").":".date("i").":".date("s")."|Pago en Una Sola Exhibicion|".$orden["subtotal"]."|".$orden["total"]."|Transferencia Electrónica|Ingreso|USD|".$configuracion["cambio"]."||".$iva."||100|El cambio de Dolares Americanos a Pesos Mexicanos es ".$configuracion["cambio"].", Orden de Compra: ".$orden["codigoexterno"].", Orden de Producción: ".$orden["codigoop"].", Identificador Fiscal: ".$empresa["identificador"]." ".$empresa["comexportacion"].". ".$orden["comfacturacion"]." |FALSE|micorreo@pruebascorreo.com|||FACTURA|".$empresa["ultimos"]."|" . PHP_EOL);
        }else{
            fwrite($file, "|COMPROBANTE|3.2|Sin Serie|".date("Y")."-".date("m")."-".date("d")." ".  date("H").":".date("i").":".date("s")."|Pago en Una Sola Exhibicion|".$orden["subtotal"]."|".$orden["total"]."|Transferencia Electrónica|Ingreso|USD|".$configuracion["cambio"]."||".$iva."||100|El cambio de Dolares Americanos a Pesos Mexicanos es ".$configuracion["cambio"].", Orden de Producción: ".$orden["codigoop"].", Identificador Fiscal: ".$empresa["identificador"]." ".$empresa["comexportacion"].". ".$orden["comfacturacion"]." |FALSE|micorreo@pruebascorreo.com|||FACTURA|".$empresa["ultimos"]."|" . PHP_EOL);
        }        
    }
